...
set BIN=%~dp0
set ROOT=%BIN%..\
set DIALOGOS_NAME=Dialogos
set DIALOGOS_EXE=%DIALOGOS_NAME%.exe
set RESULTS_DIR=results
set CONDA_ENV=Dialogos
set CONFIG_DIR=config
set BUILD_DIR=Builds\client_windows
...
set ML_AGENTS_CMD=mlagents-learn %CONFIG_DIR%\%MODEL_NAME%-%VERSION%.yaml --run-id=%MODEL_NAME%%TYPE%.%VERSION%-%SEQUENCE% --time-scale 1 --quality-level 5 --env=%BUILD_DIR%\%DIALOGOS_EXE% --num-envs=%NUM_ENVS% --no-graphics %MODE_ARG%
echo %ML_AGENTS_CMD%
call %ML_AGENTS_CMD%
if %ERRORLEVEL% neq 0 (
...
:delete_results
set RESULTS_DIR=%ROOT%\%DIALOGOS_NAME%\%RESULTS_DIR%\%MODEL_NAME%%TYPE%.%VERSION%-%SEQUENCE%
